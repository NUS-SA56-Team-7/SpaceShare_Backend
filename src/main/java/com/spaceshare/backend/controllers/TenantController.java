...
public class TenantController {
	...
	public ResponseEntity<Tenant> createTenant(@RequestBody Tenant tenant) {
		try {
			...
			if(tenantService.getTenantByEmail(tenant.getEmail()) != null){
				return new ResponseEntity<>(HttpStatus.CONFLICT);
			}

			Tenant returnTenant = tenantService.saveTenant(tenant);
	...
	public ResponseEntity<Tenant> editTenant(@PathVariable("id") UUID id, @RequestBody Tenant inTenant) {
		...
		Tenant renter = tenantService.getTenantById(id);
		...
		if (renter != null) {
			...
			Tenant updatedTenant = tenantService.updateTenant(r);
	...
	// 	List<Tenant> listTenants = tenantService.findAllTenants();

	// 	ICsvBeanWriter csvWriter = new CsvBeanWriter(response.getWriter(), CsvPreference.STANDARD_PREFERENCE);
	// 	String[] csvHeader = { "First Name", "Last Name", "Identification Number", "Address", "Phone", "Date Of Birth" };
	// 	String[] nameMapping = { "firstName", "lastName", "identificationNumber", "address", "phone", "dateOfBirth" };
